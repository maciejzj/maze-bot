...
void platformTurnAngle(int degree) {
	int actuationLeft;       ///< Actuation of the left motor, steers PWM.
	int actuationRight;      ///< Actuation of the right motor, steers PWM.
	float Pv = 1.2;          ///< P coefficient of (subordinate) velocity controller.
	float Iv = 1.2;          ///< I coefficient of (subordinate) velocity controller.
	float Pp = 0.35;         ///< P coefficient of (precedent) position controller.
	float setPointVeloRight; ///< Velocity set point for subordinate controller of right motor.
	float setPointVeloLeft;  ///< Velocity set point for subordinate controller of left motor.
	bool leftIsStopped = false;  ///< Stop indicator for left motor after reaching set point.
	bool rightIsStopped = false; ///< Stop indicator for right motor after reaching set point.
	...
	/* Set the last time ISRs as the begging of the work of the controller */
	lastTimeLeft = millis();
	lastTimeRight = millis();

	while(1) {
		int slotDeltaRight = target - motRightCounter;  ///< Error of position (precedent) controller for right motor.
		int slotDeltaLeft = target - motLeftCounter;    ///< Error of position (precedent) controller for left motor.
		...
		/* PI controller formula, calculates motors actuations */
...
void headingVeloFix() {
	...
	float Pm = 10;      ///< Proportional coefficient of master velocity controller.
	float Ps = 15;      ///< Proportional coefficient of slave velocity controller.
	float Is = 5;       ///< Integral coefficient of slave velocity controller.
